...
async function main() {
  ...
  for (const [topic, routeTopic] of topics) {
    ...
    const found = existing.webhookSubscriptions.nodes.find(
      (node) => node.topic === topic && node.endpoint?.callbackUrl === uri,
    );
    if (found) {
    ...
    if (result.userErrors.length || !result.webhookSubscription)
      throw new Error(
        `${routeTopic}: ${result.userErrors.map((error) => error.message).join("; ") || "registration failed"}`,
      );
    process.stdout.write(
      `${routeTopic}: registered (${result.webhookSubscription.id})\n`,
    );
  }
}
