...
beforeAll(() => {
  process.env.KIOTVIET_WEBHOOK_SECRET = Buffer.from(
    "kiotviet-product-secret",
  ).toString("base64");
});
beforeEach(() => {
  store.mockReset();
...
describe("KiotViet product webhook", () => {
  it("queues product changes for Shopify synchronization", async () => {
    ...
    const body = JSON.stringify({
      ...
      Attempt: 1,
      Notifications: [
        { Action: "product.update", Data: [{ ProductId: 43043436 }] },
      ],
    });
    ...
    const result = await receiveKiotVietWebhook(
      new Request("https://sync.example.com/api/kiotviet/webhooks", {
        method: "POST",
  ...
  it("queues single and bulk price changes for Shopify synchronization", async () => {
    ...
    expect(enqueue).toHaveBeenCalledWith(
      "webhooks",
      "kiotviet_product_to_shopify",
      { eventId: "event-pricebook-1" },
      "high",
      "kiotviet-kv-pricebook-1",
    );
  });

  it("queues deleted variants for Shopify synchronization", async () => {
    store.mockResolvedValueOnce({
      id: "event-product-delete",
      inserted: true,
      status: "received",
    });
    const body = JSON.stringify({
      Id: "kv-product-delete",
      Attempt: 1,
      RemoveId: [43095701],
      Notifications: [{ Action: "product.delete", Data: [] }],
    });
    const signature = createHmac(
      "sha256",
      Buffer.from(process.env.KIOTVIET_WEBHOOK_SECRET!, "base64"),
    ...
    expect(enqueue).toHaveBeenCalledWith(
      "webhooks",
      "kiotviet_product_to_shopify",
      { eventId: "event-product-delete" },
      "high",
      "kiotviet-kv-product-delete",
    );
  });

  it("queues KiotViet order updates for Shopify cancellation", async () => {
    store.mockResolvedValueOnce({
      id: "event-order-update",
      inserted: true,
      status: "received",
    });
    const body = JSON.stringify({
      ...
      Notifications: [
        {
          ...
          Data: [{ Id: 24102572, Status: 4, StatusValue: "Đã hủy" }],
        },
      ],
    });
    const signature = createHmac(
    ...
    expect(enqueue).toHaveBeenCalledWith(
      "webhooks",
      "kiotviet_order_to_shopify",
      { eventId: "event-order-update" },
      "high",
      "kiotviet-kv-order-update",
    );
  });
  ...
  it("queues category updates for Shopify collection synchronization", async () => {
    ...
    const body = JSON.stringify({
      ...
      Attempt: 1,
      Notifications: [
        { Action: "category.update", Data: [{ CategoryId: 42 }] },
      ],
    });
    const signature = createHmac(
      "sha256",
    ...
    expect(enqueue).toHaveBeenCalledWith(
      "webhooks",
      "kiotviet_category_to_shopify",
      { eventId: "event-category" },
      "high",
      "kiotviet-kv-category",
    );
  });

  it("does not infer product deletion from an untyped RemoveId", async () => {
    store.mockResolvedValueOnce({
      id: "event-remove",
      inserted: true,
      status: "received",
    ...
    expect(result.body).toMatchObject({ success: true, unsupported: true });
