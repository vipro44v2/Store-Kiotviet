...
const map: Record<string, JobType> = {
  "stock.update": "kiotviet_inventory_to_shopify",
  "pricebookdetail.update": "kiotviet_product_to_shopify",
...
export async function POST(
  ...
  try {
    ...
    const { id } = await params,
      event = await webhooksRepository.get(id);
    if (!event)
      return Response.json(
        { success: false, error: "Event not found" },
        { status: 404 },
      );
    const type = map[String(event.event_type)];
    if (!type)
      return Response.json(
