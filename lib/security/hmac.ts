...
export function verifyKiotVietHmac(
  rawBody: string,
  signature: string | null,
  base64Secret: string,
): boolean {
  ...
  const keys = [
    ...
    Buffer.from(base64Secret, "utf8"),
  ];
  return keys.some((key) => {
    const hex = createHmac("sha256", key).update(rawBody, "utf8").digest("hex");
    const base64 = createHmac("sha256", key)
      .update(rawBody, "utf8")
      .digest("base64");
    return (
      safeEqual(hex.toLowerCase(), normalizedSignature.toLowerCase()) ||
      safeEqual(base64, normalizedSignature)
    );
  });
}
