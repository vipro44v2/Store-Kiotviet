...
const topicJobs: Record<string, JobType> = {
  ...
  "fulfillments/create": "shopify_fulfillment",
  "fulfillments/update": "shopify_fulfillment",
...
export async function receiveShopifyWebhook(
  ...
  if (!webhookId)
    return {
      status: 400,
      body: { success: false, error: "Missing webhook ID" },
    };
  ...
  return { status: 200, body: { success: true, duplicate: !stored.inserted } };
}
