...
export interface KiotVietOrderSummary{id:number;code:string;description?:string}
...
export function matchKiotVietOrderByShopifyReference(orders:KiotVietOrderSummary[],shopifyOrderId:string){const reference=`(${shopifyOrderId})`;return orders.find(order=>order.description?.includes(reference));}
export async function findKiotVietOrderByShopifyReference(shopifyOrderId:string){const params=new URLSearchParams({searchTerm:shopifyOrderId,pageSize:"100",currentItem:"0"});const result=await kiotVietClient.get<{data:KiotVietOrderSummary[]}>(`/orders?${params}`);return matchKiotVietOrderByShopifyReference(result.data??[],shopifyOrderId);}
